 ...
 * Demonstrates collisions, member functions
 * and arrays
 * 
 * K Collins
 * May 12, 2017
    ...
    for (var j = 0; j < 4; j++) {
      ...
      var brick = {
        ...
        checkCollide: function() {
          var disty = Math.abs(this.y - ball.y);
          if (disty < 1 / 2 * (this.h + ball.h)) {
            var distx = Math.abs(this.x - ball.x);
            if (distx < 1 / 2 * (this.w + ball.w)) {
              // collision
              ball.dy *= -1;
              this.remove();
            }
...
function drawScore() {
  fill(255);
  textSize(32);
  text("Hits: " + numHits + " Lives: " + numLives, width / 2, 25);
}
